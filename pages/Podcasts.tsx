...
const DUMMY_PODCASTS = [
  {
    ...
    description:
      "In this episode, we explore the evolving landscape of work environments post-pandemic. Featuring insights from industry leaders, we discuss the pros and cons of remote work, hybrid models, and the future of office spaces. Learn how companies are adapting their policies and what this means for employee satisfaction and productivity.",
    cover_image_url:
  ...
  },
  {
    ...
    description:
      "Laugh along as we explore the humorous side of daily routines, awkward social situations, and the quirks that make us human. Featuring hilarious stories, celebrity interviews, and interactive segments with our listeners. A perfect dose of comedy to brighten your day and remind you not to take life too seriously.",
    cover_image_url:
...
const Podcasts = () => {
  ...
  const handleToggleLike = async (podcastId) => {
    ...
    setLikedPodcasts((prev) => {
      ...
      nowLiked ? s.add(podcastId) : s.delete(podcastId);
      return s;
    });
    setPodcasts((prev) =>
      prev.map((p) =>
        p.id === podcastId
          ? { ...p, like_count: (p.like_count || 0) + (nowLiked ? 1 : -1) }
          : p,
      ),
    );
  ...
  const handleToggleBookmark = async (podcastId) => {
    ...
    setBookmarkedPodcasts((prev) => {
      const s = new Set(prev);
      nowBookmarked ? s.add(podcastId) : s.delete(podcastId);
      return s;
    });
  ...
  return (
    <>
      ...
      >
            ...
            {CATEGORIES.map((category) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: "12px",
                  border:
                    selectedCategory === category ? "none" : "1px solid #ccc",
                  backgroundColor:
                    selectedCategory === category ? "#00BFFF" : "transparent",
        ...
        {/* FAB Button */}
        <div style={styles.fab} onClick={navigateToCreatePodcast}>

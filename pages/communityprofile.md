...
const CommunityProfile = ({ navigation, route }: Props) => {
  ...
  useEffect(() => {
    ...
    if (shouldRedirect) {
      ...
      const timer = setTimeout(() => {
        try {
          router.push("/CommunityFeed", { replace: true });
        } catch (error) {
  ...
  // Add a focus listener to refresh posts when returning to this screen
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", () => {
      if (cleanCommunityId) {
        fetchCommunityPosts();
        checkLiveStatus();
        // Update activity when user returns to community
        updateCommunityLastActive(cleanCommunityId);
      }
    });
